...
const BLOB_PIECE_LENGTH: usize = 256;
...
async fn main() -> Result<()> {
    ...
    let expected_blob_pieces = file_size / BLOB_PIECE_LENGTH as u64;
    println!(
        "▶ File: \"{}\" ({}, {} blob pieces)",
        mime_type,
    ...
    let mut reader = BufReader::with_capacity(BLOB_PIECE_LENGTH, file);
    let mut buf: [u8; BLOB_PIECE_LENGTH] = [0; BLOB_PIECE_LENGTH];
    ...
    while let Ok(size) = reader.read(&mut buf[..]).await {
        ...
        // @TODO: Use bytes operation values
